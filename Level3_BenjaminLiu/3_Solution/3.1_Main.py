...
def main():
	...
	WAM=reduce(WAM, zip(loanAmounts, loanTerms), 0)/sumWeight
	WAR=reduce(lambda total, (face, rate): total+(face*rate), zip(loanAmounts, rates), 0)/sumWeight
	...
	list_1=list(np.random.normal(10,1,20));
	list_2=list(np.random.normal(10,1,20));
	list_3=list(np.random.normal(10,1,20));
	rlist_1=reconcileLists(list_1, list_2, functools.partial(breakAbsolute, epsilon=.1));
	rlist_2=reconcileLists(list_2, list_3, functools.partial(breakRelative, percent=.1));
	rlist_3=reconcileLists(list_1, list_3, functools.partial(breakAbsRelative, percent=.1));
	print('The first with breakAbsolute is {}, \nThe second with breakRelative is {}, \nThe third with breakAbsRelative {} \n'.
		format(rlist_1, rlist_2, rlist_3))
	raw_input('Program pause. Press enter to continue.\n');
	...
	reconcileListsBreakAbsolute=functools.partial(reconcileLists, breakFn=functools.partial(breakAbsolute, epsilon=.001))
	reconcileListsBreakRelative=functools.partial(reconcileLists, breakFn=functools.partial(breakRelative, percent=.001))
	reconcileListsBreakAbsRelative=functools.partial(reconcileLists, breakFn=functools.partial(breakAbsRelative, percent=.001))
