...
# result will be that you should be able to loop over a LoanPool object’s individual Loan objects
...
Implementations can be found at 3_Solution\Implementations\Loans\LoanPool.py
==================================================================================================='''
...
def main():
	...
	for i in range(1000):
		tmp=LsIter.next()
		if (i+1)%100==0: # only print out result after every 100 iterations
			print(round(tmp, 2))
	...
	# Exercise 3.2.3
	# Modify LoanPool class to be an iterable
	print('\n====================================Exercise 3.2.3=====================================\n');
	print('Modifying LoanPool class to be an iterable ... \n');
	loanPool=LoanPool(FixedRateLoan(None, 1000000, 0.05, 36), FixedRateLoan(None, 5000000, 0.03, 12), FixedRateLoan(None, 500000, 0.06, 6));
	cnt=0
	for loan in loanPool:
		cnt+=1;
		print('The face value for loan {} is {}. \n'.format(cnt, round(loan.face, 2))); 
	raw_input('Program pause. Press enter to continue.\n');

	# Exercise 3.2.4
	print('\n====================================Exercise 3.2.4=====================================\n');
	print('Step a: Displaying the first and second values of the Fibonacci sequence ... \n');
	f=genFibo();
	...
	print('The range list compre takes time {}s; the xrange generator expression takes time {}s. \n'.
		format(round(elap, 6), round(elap2, 6)));
	...
	names=['Alex','Ben','Clare','Dave','Ford','Ginger','Henry','Jack','Kent','Ross'];
